...
const Detail = () => {
  ...
  const validate = () => {
    ...
    if (!incident.location.trim()) newErrors.location = 'Укажите место происшествия'
    if (!incident.description.trim()) newErrors.description = 'Заполните описание'
    return newErrors
  }
  ...
  const handleSubmit = e => {
    ...
    api.put(`/incidents/${id}`, JSON.stringify(incident))
      .then(() => navigate('/'))
      .catch(() => setServerError('Не удалось сохранить изменения. Попробуйте позже.'))
  }
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        <label>
          Тип инцидента:
          <input type='text' name='type' value={incident.type} onChange={handleChange} />
        </label>
        {errors.type && <div style={{ color: 'red' }}>{errors.type}</div>}
        <br />
        <label>
          Место происшествия:
          <input type='text' name='location' value={incident.location} onChange={handleChange} />
        </label>
        ...
        {currentUser.role === 'admin' && (
          <>
            <label>
              Назначен расследователю (email):
              <input type='text' name='assignedTo' value={incident.assignedTo} onChange={handleChange} />
            </label>
            <br />
          </>
        )}
